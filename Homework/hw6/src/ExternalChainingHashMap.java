...
public class ExternalChainingHashMap<K, V> {
    ...
    public V put(K key, V value) {
        if (key == null || value == null) {
            throw new IllegalArgumentException("Key or value does not exist");
        }
        ...
        while (curr != null) { // key exists
            if (curr.getKey().equals(key)) { //duplicate key
                val = curr.getValue();
                curr.setValue(value);
                return val;
            }
    ...
    public void resizeBackingTable(int length) {
        ...
        for (int i = 0; i < table.length; i++) {
            ExternalChainingMapEntry<K, V> curr = table[i];
            while (curr != null) {
                newIndex = Math.abs(curr.getKey().hashCode() % newTable.length);
                newTable[newIndex] = new ExternalChainingMapEntry<>(curr.getKey(), curr.getValue(), newTable[newIndex]);
                curr = curr.getNext();
            }
        }
        table = newTable;
